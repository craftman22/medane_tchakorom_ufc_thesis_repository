...
PetscErrorCode comm_async_probe_and_receive_prime(Vec *x_block_jacobi,
                                                  ...
                                                  VecScatter *scatter_ctx, Vec NewerDependencies_local, const PetscInt proc_local_rank)
{
    PetscFunctionBeginUser;
    MPI_Status status;
    PetscMPIInt rcv_data_flag;
    PetscInt pack_buffer_size;
    // PetscInt SrcTag = 0;
    // PetscInt SrcNode = 0;
    PetscInt SrcPhaseTag = -1;
    PetscInt SrcCurrentIteration = -1;
    PetscBool RcvMessage = PETSC_FALSE;
    ...
    if (rcv_data_flag)
    {
        ...
        do
        {
            PetscCallMPI(MPI_Recv((*pack_buffer), pack_buffer_size, MPI_PACKED, message_source, TAG_MULTISPLITTING_DATA, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
            PetscCallMPI(MPI_Iprobe(message_source, TAG_MULTISPLITTING_DATA, MPI_COMM_WORLD, &rcv_data_flag, MPI_STATUS_IGNORE));
        } while (rcv_data_flag);
    ...
    PetscCall(VecAssemblyBegin(NewerDependencies_global));
    PetscCall(VecAssemblyEnd(NewerDependencies_global));

    PetscCall(VecScatterBegin((*scatter_ctx), NewerDependencies_global, NewerDependencies_local, INSERT_VALUES, SCATTER_FORWARD));
    PetscCall(VecScatterEnd((*scatter_ctx), NewerDependencies_global, NewerDependencies_local, INSERT_VALUES, SCATTER_FORWARD));

    PetscFunctionReturn(PETSC_SUCCESS);
}

PetscErrorCode comm_async_test_and_send_prime(PetscInt PhaseTag_param, PetscInt NumIteration_param, Vec *x_block_jacobi,
                                              PetscScalar *send_buffer, MPI_Request *send_data_request, PetscMPIInt vec_local_size,
                                              PetscMPIInt message_dest, PetscMPIInt rank_jacobi_block, char **pack_buffer)
{
    PetscFunctionBeginUser;

    PetscMPIInt position = 0;
    PetscMPIInt flag = 1;
    const PetscScalar *temp_buffer = NULL;

    if ((*send_data_request) != MPI_REQUEST_NULL)
        PetscCallMPI(MPI_Test(send_data_request, &flag, MPI_STATUS_IGNORE));

    if (flag)
    {
        PetscCall(VecGetArrayRead(x_block_jacobi[rank_jacobi_block], &temp_buffer));
        PetscCall(PetscArraycpy(send_buffer, temp_buffer, vec_local_size));
        PetscCall(VecRestoreArrayRead(x_block_jacobi[rank_jacobi_block], &temp_buffer));
        PetscCall(pack_computed_data_dependency(&PhaseTag_param, &NumIteration_param, vec_local_size, send_buffer, pack_buffer, &position));
        PetscCallMPI(MPI_Isend((*pack_buffer), position, MPI_PACKED, message_dest, TAG_MULTISPLITTING_DATA, MPI_COMM_WORLD, send_data_request));
    }
