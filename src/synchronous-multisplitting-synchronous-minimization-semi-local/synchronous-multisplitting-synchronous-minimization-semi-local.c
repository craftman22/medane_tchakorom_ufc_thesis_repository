...
int main(int argc, char **argv)
{
  ...
  KSP inner_ksp = NULL;
  KSP outer_ksp = NULL;
  PetscMPIInt nlocal_rows_x_block = ZERO;
  PetscMPIInt nlocal_rows_x = ZERO;
  ...
  // PetscCall(initializeKSP(comm_jacobi_block, &outer_ksp, R, rank_jacobi_block, PETSC_TRUE, OUTER_KSP_PREFIX, OUTER_PC_PREFIX));
  ...
  // PetscScalar initial_global_residual_norm;
  // PetscCall(computeFinalResidualNorm(A_block_jacobi, x_minimized, b_block_jacobi, rank_jacobi_block, proc_local_rank, &initial_global_residual_norm));
  // PetscCall(printFinalResidualNorm(initial_global_residual_norm));
  ...
  PetscCall(computeFinalResidualNorm(comm_jacobi_block, comm_local_roots, A_block_jacobi, x_minimized, b_block_jacobi, local_residual, rank_jacobi_block, proc_local_rank, &global_norm_0));
  ...
  do
  {
    ...
    while (basis_vector_i < s)
    {
      ...
      // PetscCall(MatSetValuesLocal(S, x_local_size, vec_local_idx, ONE, &basis_vector_i, vector_to_insert_into_S, INSERT_VALUES));
    ...
    PetscCall(VecScatterBegin(scatter_jacobi_vec_part_to_merged_vec[idx_non_current_block], x_minimized, x_block_jacobi[idx_non_current_block],
                              INSERT_VALUES, SCATTER_REVERSE));

    PetscCall(VecScatterEnd(scatter_jacobi_vec_part_to_merged_vec[idx_non_current_block], x_minimized, x_block_jacobi[idx_non_current_block],
                            INSERT_VALUES, SCATTER_REVERSE));

    PetscCall(VecScatterBegin(scatter_jacobi_vec_part_to_merged_vec[rank_jacobi_block], x_minimized, x_block_jacobi[rank_jacobi_block],
                              INSERT_VALUES, SCATTER_REVERSE));

    PetscCall(VecScatterEnd(scatter_jacobi_vec_part_to_merged_vec[rank_jacobi_block], x_minimized, x_block_jacobi[rank_jacobi_block],
                            INSERT_VALUES, SCATTER_REVERSE));
  ...
  PetscCall(PetscPrintf(comm_jacobi_block, "[Block rank %d] Erreur : %e \n", rank_jacobi_block, error));
...
// PetscCall(outer_solver_lsqr(comm_jacobi_block, outer_ksp, x_minimized, R, S, R_transpose_R, vec_R_transpose_b_block_jacobi, alpha, b_block_jacobi[rank_jacobi_block], rank_jacobi_block, s, number_of_iterations));
// PetscCall(outer_solver_lsqr(comm_jacobi_block, outer_ksp, x_minimized, R, S, alpha, b_block_jacobi[rank_jacobi_block], rank_jacobi_block, number_of_iterations));

////
// PetscCall(VecWAXPY(tmp_diff, -1.0, tmp, x_block_jacobi[rank_jacobi_block]));
// PetscCall(VecNorm(tmp_diff, NORM_INFINITY, &tmp_norm));
// PetscCall(PetscPrintf(comm_jacobi_block, "[ Block rank %d ][ outer iter %d] --------------- tmp norm = %e \n", rank_jacobi_block, number_of_iterations, tmp_norm));
///
...
// PetscCall(outer_solver_norm_equation(comm_jacobi_block, outer_ksp, x_minimized, R, S, alpha, b_block_jacobi[rank_jacobi_block], rank_jacobi_block, number_of_iterations));
